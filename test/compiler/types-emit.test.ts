...
describe("emitSlabDts", () => {
  it("emits declare module, aggregate type, and shader exports", () => {
...
COLOR = vec4(UV, 0.0, 1.0);
    ...
    expect(dts).toContain("export interface HelloSlabShaders");
    expect(dts).toContain("readonly demo: ShaderInstance<DemoShaderUniforms>");
    expect(dts).toContain("export declare const demo: ShaderInstance<DemoShaderUniforms>");
  ...
  it("emits range JSDoc, vec tuples, and readonly mouse_position", () => {
    const src = `<shaderlab version="1.0">
  <shader id="main" type="canvas_item">
    <uniforms>
      <uniform name="strength" type="float" hint="range(0.0, 0.05)" default="0.01" />
      <uniform name="mouse_uv" type="vec2" hint="mouse_position" />
      <uniform name="tint" type="vec3" hint="color" />
    </uniforms>
    <fragment><![CDATA[
COLOR = vec4(UV, 1.0);
    ]]></fragment>
    ...
    expect(dts).toContain("export interface PanelSlabShaders");
  });
});
